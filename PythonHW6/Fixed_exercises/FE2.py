...
def not_even_nums_sum():
    try:
        new_list = list(map(int, input("Enter numbers without spaces: ")))
        print("\033[32m{}".format(f"list --> {new_list}"))
        result_list = list(filter(lambda x: not x%2, new_list))
        num_sum = sum(result_list)
        print("\033[1m\033[32m{}".format(f"sum --> {num_sum}"))
    except ValueError:
        print("\033[1m\033[31m{}".format("You must enter an integer!"))
